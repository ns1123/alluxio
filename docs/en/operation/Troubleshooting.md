...
worker storage, and can be evicted when Alluxio worker storage is full and some new data needs to be cached. To make sure data is persisted,
either use `CACHE_THROUGH` or `THROUGH` write type, or `ASYNC_THROUGH` with alluxio.user.file.replication.durable` set to an acceptable degree of redundancy.
<!-- ALLUXIO CS END -->
