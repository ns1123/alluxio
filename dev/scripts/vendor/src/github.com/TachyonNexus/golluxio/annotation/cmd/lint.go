...
var cmdLint = &cmdline.Command{
	Name:   "lint",
	Short:  "Checks validity of closed source annotations",
	Long:   "Checks validity of closed source annotations.",
...
func lint(filename string, warnings map[string][]warning) error {
	...
	for scanner.Scan() {
		line++
		for _, warning := range sm.next(scanner.Text(), line, ct) {
...
func runLint(env *cmdline.Env, args []string) error {
	...
	if len(warnings) != 0 && flagFailOnWarning {
