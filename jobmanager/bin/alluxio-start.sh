...
start_worker() {
  ...
  echo "Starting job manager worker @ `hostname -f`. Logging to $ALLUXIO_LOGS_DIR"
  (nohup $JAVA -cp $CLASSPATH -Dalluxio.home=$ALLUXIO_HOME -Dalluxio.logs.dir=$ALLUXIO_LOGS_DIR -Dalluxio.logger.type="WORKER_LOGGER" -Dalluxio.accesslogger.type="WORKER_ACCESS_LOGGER" -Dlog4j.configuration=file:$ALLUXIO_CONF_DIR/log4j.properties $ALLUXIO_WORKER_JAVA_OPTS alluxio.worker.AlluxioJobManagerWorker > $ALLUXIO_LOGS_DIR/job_worker.out 2>&1 ) &
}
...
restart_worker() {
  ...
  RUN=`ps -ef | grep "alluxio.worker.AlluxioJobManagerWorker" | grep "java" | wc | cut -d" " -f7`
  if [[ $RUN -eq 0 ]] ; then
    echo "Restarting worker @ `hostname -f`. Logging to $ALLUXIO_LOGS_DIR"
    (nohup $JAVA -cp $CLASSPATH -Dalluxio.home=$ALLUXIO_HOME -Dalluxio.logs.dir=$ALLUXIO_LOGS_DIR -Dalluxio.logger.type="WORKER_LOGGER" -Dalluxio.accesslogger.type="WORKER_ACCESS_LOGGER" -Dlog4j.configuration=file:$ALLUXIO_CONF_DIR/log4j.properties $ALLUXIO_WORKER_JAVA_OPTS alluxio.worker.AluxioJobManagerWorker > $ALLUXIO_LOGS_DIR/job_worker.out 2>&1) &
  fi
}
...
run_safe() {
  while [ 1 ]
  do
    RUN=`ps -ef | grep "alluxio.master.AlluxioJobManagerMaster" | grep "java" | wc | cut -d" " -f7`
    if [[ $RUN -eq 0 ]] ; then
      echo "Restarting the Alluxio job manager master..."
      start_master
    fi
    echo "Alluxio job manager is running... "
    sleep 2
  done
}
