...
/**
 * A netty packet reader with encryption.
 * TODO(chaomin): this is just a prototype, need to dedup the logic with {@link NettyPacketReader}.
 */
@NotThreadSafe
// TODO(chaomin): simple read encrypted file works, need to fix the FileInStreamIntegrationTest
public final class CryptoNettyPacketReader implements PacketReader {
  private static final Logger LOG = LoggerFactory.getLogger(CryptoNettyPacketReader.class);
  ...
  private static final long PHYSICAL_CHUNK_SIZE =
  ...
  /** Physical packet size to request from the data server. */
  private final long mPhysicalPacketSize;
  /** Logical bytes left to read. */
  private long mBytesLeft;
  /** The physical pos to read at. */
  private long mPhysicalPosToRead;
  /** The logical offset from the logical chunk start. */
  private long mOffsetFromChunkStart;
  ...
  /**
   * Creates an instance of {@link CryptoNettyPacketReader}. If this is used to read a block
   * remotely, it requires the block to be locked beforehand and the lock ID is passed to
   * this class.
   *
   ...
   */
  private CryptoNettyPacketReader(
      FileSystemContext context, InetSocketAddress address, long id, long offset, long len,
    ...
    mLayoutSpec = new LayoutSpec(0 /* packet header */, 0 /* packet footer */, PACKET_SIZE,
        CHUNK_HEADER_SIZE, CHUNK_SIZE, CHUNK_FOOTER_SIZE);
    // Request packet size should be physical.
    mPhysicalPacketSize = LayoutUtils.toPhysicalLength(mLayoutSpec, 0, PACKET_SIZE);
    ...
    mRequestType = type;
    mNoCache = noCache;
    mPhysicalPosToRead = LayoutUtils.getPhysicalChunkStart(mLayoutSpec, offset);
    mOffsetFromChunkStart = LayoutUtils.getPhysicalOffsetFromChunkStart(mLayoutSpec, offset);
    // Always read a total length that is multiple of the physical chunk size, otherwise it
    // is not possible to decrypt.
    long physicalReadLength = (mOffsetFromChunkStart + len + CHUNK_SIZE - 1) / CHUNK_SIZE
        * PHYSICAL_CHUNK_SIZE;
    mBytesLeft = len;
    ...
    Protocol.ReadRequest readRequest =
        Protocol.ReadRequest.newBuilder().setId(id).setOffset(mPhysicalPosToRead)
            .setLength(physicalReadLength).setLockId(lockId).setSessionId(sessionId).setType(type)
            .setNoCache(noCache).setPacketSize(mPhysicalPacketSize).build();
   ...
   */
  // TODO(chaomin): dedup this with {@link NettyPacketReader#PacketReadHandler}
  ...
  /**
   * Factory class to create {@link CryptoNettyPacketReader}s.
   */
  public static class Factory implements PacketReader.Factory {
    ...
    /**
     * Creates an instance of {@link CryptoNettyPacketReader.Factory} for block reads.
     *
     * @param context the file system context
    ...
    public PacketReader create(long offset, long len) throws IOException {
      return new CryptoNettyPacketReader(mContext, mAddress, mId, offset, len, mLockId, mSessionId,
