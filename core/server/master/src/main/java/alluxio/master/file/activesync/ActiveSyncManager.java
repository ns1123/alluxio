 ...
 */
@NotThreadSafe
public class ActiveSyncManager implements JournalEntryIterable {
   ...
   */
  public void applyAndJournal(RpcContext context, AddSyncPointEntry entry) {
    try {
      apply(entry);
      context.journal(Journal.JournalEntry.newBuilder().setAddSyncPoint(entry).build());
    } catch (Throwable t) {
   ...
   */
  public void applyAndJournal(RpcContext context, RemoveSyncPointEntry entry) {
    try {
      apply(entry);
      context.journal(Journal.JournalEntry.newBuilder().setRemoveSyncPoint(entry).build());
    } catch (Throwable t) {
      ProcessUtils.fatalError(LOG, t, "Failed to apply %s", entry);
      throw t; // fatalError will usually system.exit
    }
