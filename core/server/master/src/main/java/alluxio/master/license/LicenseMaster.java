...
public class LicenseMaster extends AbstractMaster {
  ...
  public LicenseMaster(MasterRegistry registry, MasterContext masterContext) {
    super(masterContext, new SystemClock(), ExecutorServiceFactories
        .fixedThreadPool(Constants.LICENSE_MASTER_NAME, 2));
   ...
   */
  private void setLastCheckSuccess(long time) throws UnavailableException {
    ...
    public void heartbeat() {
      ...
      if (isValid) {
        ...
        try {
          ...
          LOG.warn("Failed to journal license check: {}", e.toString());
        }
      } else {
