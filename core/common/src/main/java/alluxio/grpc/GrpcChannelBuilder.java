...
public final class GrpcChannelBuilder {
  ...
  private GrpcChannelBuilder(GrpcServerAddress address, AlluxioConfiguration conf) {
    ...
    // ALLUXIO CS ADD
    if (mConfiguration.getBoolean(alluxio.conf.PropertyKey.NETWORK_TLS_ENABLED)) {
      sslContext(alluxio.util.network.SSLUtils.getClientSslContext(mConfiguration));
    }
    // ALLUXIO CS END
  }
  ...
  public GrpcChannelBuilder sslContext(io.netty.handler.ssl.SslContext sslContext) {
    mChannelKey.setSslContext(sslContext);
    return this;
  }
