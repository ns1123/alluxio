...
public abstract class ThriftClientPool<T extends AlluxioService.Client>
  ...
  protected T createNewResource() {
    ...
    // ALLUXIO CS WITH
    AuthenticatedThriftProtocol authenticatedThriftProtocol =
        new AuthenticatedThriftProtocol(binaryProtocol, mServiceName);
    T client = createThriftClient(authenticatedThriftProtocol);
    // ALLUXIO CS END
    ...
    do {
      ...
      try {
        if (!transport.isOpen()) {
          ...
          // transport.open();
          // ALLUXIO CS WITH
          authenticatedThriftProtocol.openTransport();
          // ALLUXIO CS END
        }
        if (transport.isOpen()) {
          checkVersion(client);
